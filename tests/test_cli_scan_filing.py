...
def test_qr_routed_scoring_files_scan_after_routed_export(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(cli, "process_file_qr_aware", lambda input_file: [_result()])
    ...
    monkeypatch.setattr(cli, "export_routed_results", export_routed)
    monkeypatch.setattr(cli, "file_original_scan_copy", file_scan)
    monkeypatch.setattr(cli, "print_scan_filing_result", lambda result: None)
    monkeypatch.setattr(cli, "print_qr_batch_summary", lambda summary: None)
    monkeypatch.setattr(cli, "save_qr_batch_summary", lambda summary, source: None)
...
def test_explicit_output_csv_skips_scan_filing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(cli, "process_file_qr_aware", lambda input_file: [_result()])
    monkeypatch.setattr(cli, "export_to_csv", lambda results, output_file: True)
    monkeypatch.setattr(
        cli,
...
def test_manual_scoring_skips_scan_filing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(cli, "load_answer_key", lambda path: {1: "A"})
    monkeypatch.setattr(cli, "process_file", lambda input_file, key: [_result()])
    monkeypatch.setattr(cli, "export_to_csv", lambda results, output_file: True)
    monkeypatch.setattr(
        cli,
        "file_original_scan_copy",
