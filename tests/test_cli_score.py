...
def test_qr_aware_no_scored_pages_prints_and_saves_summary(monkeypatch, capsys):
    calls = []
    results = Results()
    ...
    monkeypatch.setattr(
        ...
        lambda input_file: calls.append(("process", input_file)) or results,
    )
    monkeypatch.setattr(
        cli_score,
    ...
    monkeypatch.setattr(
        ...
        lambda summary, source: calls.append(("save_summary", summary, source)),
    )
...
def test_qr_aware_export_failure_updates_summary_and_skips_filing(
    ...
    monkeypatch.setattr(cli_score, "process_file_qr_aware", lambda input_file: results)
    monkeypatch.setattr(cli_score, "export_routed_results", lambda all_results: False)
    monkeypatch.setattr(
        cli_score,
        "update_qr_batch_result_write_status",
        lambda all_results, export_success, output_file=None: calls.append(
            ("write_status", all_results, export_success, output_file)
        ),
    )
    monkeypatch.setattr(
        cli_score,
    ...
    monkeypatch.setattr(
        ...
        lambda all_results, source: calls.append(("file", all_results, source)),
    )

    assert cli_score.run_score(["scan.pdf"]) == 1

    assert calls == [
        ("write_status", results, False, None),
        ("summary", results),
        ("print_summary", "summary"),
        ("save_summary", "summary", "scan.pdf"),
    ]
